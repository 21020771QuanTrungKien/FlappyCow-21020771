...
bool BlockObj::LoadImg(std::string path, SDL_Renderer* screen)
{
    bool ret = BaseObj::LoadImageFile(path, screen);
    return ret;
}

void BlockObj::SetPos(const int& xp, const int& yp)
{
    this->SetRect(xp, yp);
}

void BlockObj::SetXPos(const int& xp)
{
    SDL_Rect rect = this->GetRect();
    this->SetRect(xp, rect.y);
}

void BlockObj::ShowImg(SDL_Renderer* screen)
{
    this->Render(screen);
}

void BlockObj::DoRun(UINT& x_val)
{
    this->rect_.x += x_val;
...
{
    int number = SDLCommonFunc::Random_(1, 10);
    int number2 = number;
    number = number * 14;
...
{
    m_Topblock.DoRun(x_value);
    m_BottomBlock.DoRun(x_value);
...
{
    bool ret = false;
    ret = SDLCommonFunc::Collision(rect, slot_rect_);
    return ret;
}

bool DoubleBlock::CheckCol(SDL_Rect pl_rect)
{
    bool ret1 = SDLCommonFunc::Collision(pl_rect, m_Topblock.GetRectPos());
    bool ret2 = SDLCommonFunc::Collision(pl_rect, m_BottomBlock.GetRectPos());
...
void BlockManager::Render(SDL_Renderer* screen)
{
    for (int i = 0; i < m_BlockList.size(); i++)
    {
        ...
        if (!stop_moving_)
        {
            ...
            {
                is_col_ = true;
                Mix_Chunk* beep_sound = Mix_LoadWAV("sound//death.wav");
                if (beep_sound != NULL)
