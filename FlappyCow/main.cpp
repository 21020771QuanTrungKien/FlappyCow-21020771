...
bool InitData()
{
    ...
    if (g_window != NULL)
    {
        ...
        g_font_text = TTF_OpenFont("font//F.ttf", 38);
        if (g_font_text == NULL)
        {
            return false;
        }

        g_font_MENU = TTF_OpenFont("font//F.ttf", 80);
        if (g_font_MENU == NULL)
        {
            return false;
        }
...
bool LoadBackground()
{
    bool ret = g_background.LoadImageFile("img//bgr.png", g_screen);
    return ret;
}
...
int main(int argc, char* argv[])
{
    ...
    //Make menu game 
    int ret_menu = SDLCommonFunc::ShowMenu(g_screen, g_font_MENU, "Play now", "Exit", "img//MENU.png");
    if (ret_menu == 1)
        quit = true;
...
again_label:
    ...
    TextObj text_count_;
    text_count_.setColor(TextObj::WHITE_TEXT);
